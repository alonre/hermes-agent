    ...
    async def _handle_ws_event(self, event: Dict[str, Any]) -> None:
        ...
        if not is_dm:  # DMs need no gating; channels are mention-gated.
            ...
            if message_text is None:
                return
        # Thread support: use root_id for thread replies, or post_id for top-level channel posts
        # so subsequent thread replies continue the same session. DMs keep a single session (no
        # per-thread isolation).
        thread_id = None if is_dm else (post.get("root_id") or post_id or None)
